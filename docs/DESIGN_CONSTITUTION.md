...
are new in Milestone 12, generated by the same `scripts/gen_cursors.py` technique (a logical art
grid — 16×16 for five of the six roles, widened to 20×20 for Hourglass in M20C1R4 to give its
taper room — nearest-neighbor scale, navy fill with a 1px white halo). M20C1 changed the scale
from 4× to 2× and crops each PNG to its glyph plus outline, so a cursor sits in the size class of
an OS pointer: Arrow 18×28, Hand 32×32 (M20C1R3), I-Beam 16×28, Resize 28×28, Move 30×30,
Hourglass 32×40 (M20C1R4) CSS px.
The Hand is M20C1R3 artwork, redrawn against a supplied early-retro reference image (shape only —
CATTIPU's own navy/white, hard-edge, no-antialiasing construction is unchanged): four fingers step
down left to right — index, middle, ring, pinky — each a row shorter than the last and separated by
a 1-cell crease, with a rectangular thumb block that visibly protrudes past the palm's own left
edge for two rows before retreating into a tapered, cuffed base.
The Hourglass is M20C1R4 artwork, traced closely off a second supplied reference image and redrawn
as a genuine HOLLOW frame rather than a solid silhouette: a flat cap, a straight-sided glass
"shoulder" held flush for two rows under each cap, then a single-cell-wide diagonal wall tapering
one column per row down to a solid neck, and a solid sand-pile bump on the bottom cap rather than a
mirrored top. The hollow chamber under each cap reads white/transparent through the frame — the
same "halo() fills any enclosed transparent cell" technique Move already used for its arrowhead
notches, not a new mechanism.
The earlier 64×64 PNGs rendered 2.5–3× that size. Cropping also keeps Chromium's native-cursor
fallback zone (it drops a custom cursor that does not fit wholly inside the viewport) as small as
the artwork allows. The generator computes hotspots from the artwork: the Arrow's tip, the Hand's
fingertip, and the visual centre for the rest. Move replaces a disclosed
