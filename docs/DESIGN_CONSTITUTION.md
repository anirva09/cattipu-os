...
pointer: Arrow 18×28, Hand 30×28 (M20C1R2), I-Beam 16×28, Resize 28×28, Move 30×30,
Hourglass 28×28 (M20C1R2) CSS px.
The Hand is M20C1R2 artwork: an index finger raised left of centre, two folded-finger knuckles,
and a thumb drawn as its own 4-cell lobe on the lower left — separated from the fist by a single-
row web crease, then merged solid into the palm for the rest of its height — a pointing hand,
never a single raised finger, with a thumb that reads as a thumb rather than a rounding artifact
of the palm outline. The Hourglass is also M20C1R2 artwork: the topmost and bottommost rows widen
by one cell on each side, in place, so the frame reads as a cap sitting proud of the diagonal glass
taper beneath it, rather than the taper running straight to the silhouette's own point.
