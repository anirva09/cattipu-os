...
grid, nearest-neighbor scale, navy fill with a 1px white halo). M20C1 changed the scale from 4× to
2× and crops each PNG to its glyph plus outline, so a cursor sits in the size class of an OS
pointer: Arrow 18×28, Hand 32×32 (M20C1R3), I-Beam 16×28, Resize 28×28, Move 30×30,
Hourglass 32×32 (M20C1R3) CSS px.
...
edge for two rows before retreating into a tapered, cuffed base. The Hourglass is also M20C1R3
artwork, redrawn against the same reference: a flat cap, a one-cell-inset flange row, then a
straight linear taper down to a 2-cell neck — a crisp diagonal silhouette, not the previous rounded
taper — and an asymmetric lower half (it skips one widening step and gets a second cap row) so the
base reads as a piled mound rather than a mirrored spool. Still a solid silhouette, not a hollow
outline, per the "strong silhouette" requirement at cursor size.
