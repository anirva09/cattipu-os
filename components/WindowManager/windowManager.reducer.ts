...
export function windowManagerReducer(
  ...
  switch (action.type) {
    ...
    case 'arrange': {
      ...
      // does (CATTIPU_WINDOW_MIN_SIZE). Tile is attempted first. Cascade,
      // and Tile when no grid can hold these windows at their minimums,
      // share one geometry path: the fitted cascade, called as a pure
      // function rather than dispatched as a second command.
      ...
      const rects: WindowRect[] =
        ...
        fittedCascadeLayout(mins, action.bounds, CATTIPU_DEFAULT_WINDOW_SIZE);
